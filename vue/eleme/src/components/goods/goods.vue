<template>
  <div class="goods">
       <div class="sidebar" ref="menuWrapper">
          <ul>
            <li v-for="(item,index) in goods" class="menu-item" :class="{'cur': currentIndex === index}"  @click="selectMenu(index,$event)">
              <!--<a href="">{{item.name}}</a>-->
           {{item.name}}
            </li>
          </ul>
       ...
       <div class="goods-content" ref="foodsWrapper">
           <ul>
              <li v-for="good in goods" class="food-item" v-bind:class="currentIndex? 'cur':'' ">
